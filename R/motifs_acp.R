...
motifs_acp <-
  function(path = "~/Dropbox/2020-2021/Motifs/",
    ...
    if (n_obs == "all") {
      corpus_PCA <-
        prcomp(corpus_norm[1:nrow(corpus_norm), ], scale. = FALSE)
    }
    else {
    ...
    plot_bis <- fviz_pca_biplot(corpus_PCA,
                                repel = TRUE,
                                col.var = "#2E9FDF",
                                col.ind = "#696969")
